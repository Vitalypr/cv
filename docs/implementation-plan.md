...
- **Time budget made graphical (v2.1, product-owner request):** the "how much is left to fill" line became a shared `TimeBudgetBar` — three figures (עבדת / מולא / נותר) over a full-width bar, used by both the day card and every session card. Petrol while filling, green when it balances, red across the whole track when over-allocated with the third figure naming the excess. Exact values stay as text beside the bar, so the colour is never the only source; snapshots lock all three states.
